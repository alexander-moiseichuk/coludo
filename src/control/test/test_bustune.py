...
Host (CPython) test for the `bustune` operator command: it drives a fake board's `bustune` ladder
...
from commands import bustune  # noqa: E402
...
def _run(hub, tokens):
    [line] = asyncio.run(bustune.bustune_command(hub, tokens, {}))
    return line
...
def test_clean_sweep_keeps_ceiling():
    ...
    out = json.loads(_run(_Hub(board), ['bustune', 'taster', 'i2c', '0'])[len('from cc ok '):])
...
def test_limited_sweep_backs_off_and_names_limiter():
    ...
    out = json.loads(_run(_Hub(board), ['bustune', 'taster', 'spi', '1'])[len('from cc ok '):])
    ...
    out2 = json.loads(_run(_Hub(board2), ['bustune', 'taster', 'spi', '1', '2'])[len('from cc ok '):])
    assert out2['chosen'] == 8000000


def test_guards():
    # bad kind / missing args -> badargs; an offline board -> noboard
    assert 'badargs' in _run(_Hub(_FakeBoard('i2c', '0', 400000)), ['bustune', 'taster', 'usb', '0'])
    assert 'badargs' in _run(_Hub(_FakeBoard('i2c', '0', 400000)), ['bustune', 'taster'])
