...
class Guidance:
    ...
    def __init__(self, config: GuidanceConfig, mission, governor, position, agl):
    ...
    def _steer(self, stage: int, setpoint: dict, heading: float, now_us: int) -> bool:
        ...
        self.heading_error = heading_error(self._target_heading(heading, final, now_us), heading)
        self.roll_setpoint = fixed.from_float(setpoint.get('roll', 0.0))
        self.pitch_setpoint = fixed.from_float(setpoint.get('pitch', 0.0))
        if config.land_bank_gain and (final or stage == _STAGE.LANDING):
            # final approach / landing: FULL fin authority to crab the crosswind out; the residual
            # at strong wind is airframe-bound, not a control gap.
    ...
    def _target_heading(self, heading: float, final: bool, now_us: int) -> float:
        """The heading to steer in GLIDING / LANDING. High on the glide it homes to the zone
        (navigation.steer: gate -> centre); low on FINAL approach it TRACKS the strip centreline
        (navigation.approach), so a crosswind is crabbed out before the narrow touchdown. Tiers when
        homing:
