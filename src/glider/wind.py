...
class WindEstimator(inspector.Inspectable):
    ...
    def observe(self, course: float, ground_speed: float, airspeed: float, heading: float) -> None:
        """Fold in one GNSS fix. `course`/`heading` deg, `ground_speed`/`airspeed` m/s. A per-fix estimate
        beyond the wind envelope (max_speed) is rejected as a GNSS jump rather than folded in."""
