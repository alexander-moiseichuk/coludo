# On-board test for the landing-zone navigation geometry (nav.py): bearing/distance, zone -> target +
...
def test_bearing_distance():
    assert _close(nav.bearing(_LAT, _LON, _LAT + 0.001, _LON), 0.0)      # due north
    assert _close(nav.bearing(_LAT, _LON, _LAT, _LON + 0.001), 90.0)     # due east
    assert _close(nav.bearing(_LAT, _LON, _LAT - 0.001, _LON), 180.0)    # due south
    assert _close(nav.bearing(_LAT, _LON, _LAT, _LON - 0.001), 270.0)    # due west
    assert _close(nav.distance(_LAT, _LON, _LAT + 0.001, _LON), 111.32, 0.5)  # 0.001 deg lat
    # longitude metres shrink by cos(lat) (~0.669 here), so the same deg east is shorter than north
    assert nav.distance(_LAT, _LON, _LAT, _LON + 0.001) < nav.distance(_LAT, _LON, _LAT + 0.001, _LON)


def test_zone_orientation():
    # horizontally (longitude) stretched: 0.001 deg tall x 0.010 deg wide -> gates on left/right edges
    target, gate_a, gate_b = nav.zone((48.001, 11.000), (48.000, 11.010))
    ...
    target, gate_a, gate_b = nav.zone((48.010, 11.000), (48.000, 11.001))
...
def test_inside_and_steer():
    ...
    assert nav.inside((48.0005, 11.005), tl, br) is True  # the centre is inside
    assert nav.inside((48.0005, 10.990), tl, br) is False  # west of the zone

    # west of the zone -> head for the LEFT gate (the nearer entrance), leg 'gate', bearing ~east
    heading, waypoint, leg = nav.steer((48.0005, 10.990), tl, br)
    assert leg == 'gate' and _close(waypoint[1], 11.000, 1e-4) and _close(heading, 90.0, 5.0)

    # east of the zone -> head for the RIGHT gate
    heading, waypoint, leg = nav.steer((48.0005, 11.020), tl, br)
    ...
    heading, waypoint, leg = nav.steer((48.0005, 11.002), tl, br)
...
def test_overshoot_loop():
    ...
    heading, waypoint, leg = nav.steer((48.0005, 11.011), tl, br)  # just past the east edge
...
test_overshoot_loop()
print('ok: nav -- bearing/distance, zone target + gates, inside, steer (gate->target), overshoot loop')
