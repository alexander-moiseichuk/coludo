...
def default() -> dict:
    ...
    networks: FULL per-network configs, each entry the proven panda shape replicated -- ssid +
    optional enabled (default true) / policy / retry_ms / tx_power_dbm / password, missing keys
    inherited from this top-level section. retry_ms is THAT network's own minimal re-attempt
    interval (per-network backoff). Scanning runs through SETTING, silent BOOSTING->LANDING,
    resumes at DONE. Passwords per SSID from <ssid>.creds (panda.creds, coludo.creds); `password`
    ...
    """
    i2c:1 -- the ESP32-P4's second I2C (audio-codec bus, codec unused). Hosts the INA226 on the aft
    power board on its own short local bus, so i2c:0 (the forward sensor cluster) stays short/fast
    (see doc/board_layout.md). GPIOs 30/31 are real header pins (the codec's own 9-13 are not
    ...
    """
    ADXL375 on its own SPI bus (mode 3, 5 MHz). NOTE the Adafruit 5374 breakout labels its SPI data
    pins by their I2C names: SDA = MOSI (->47), SDO = MISO (->46). See doc/hardware.md
    "ADXL375 -> SPI wiring".
    """
    spi_buses = {
    ...
    # (lower first); several providers per quantity is normal (different drivers, redundancy).
    ...
    the filter trusts the gyro alone; below it, straight-ish flight lets the gravity vector
    re-anchor roll/pitch and cancel gyro drift.
    """
    attitude = {
    ...
    """
    GNSS. Primary: ATGM336H (CASIC) at 10 Hz. Backup: GY-NEO6MV2 (u-blox NEO-6M) -- same uart:2,
    ...
    # ---- components: consumers / actuators / system tasks. `driver` runs from drivers/ (HAL),
    ...
    """
    Flight-stage automation: launch-detect -> separation/burnout -> agl-landing -> on-ground. Drives
    the stage machine the control loop gates on; logs every transition (+ sequencer.csv). Enabled --
    safe on the passive flights (the flight task is the actuator and stays disabled), and it captures
    the stage timeline. Tune launch_g/launch_ms from the first powered flights.
    ...
    deploy at apogee: apogee_drop_m (baro fell 5 m off its peak) is the primary boost->glide trigger
    after the separation switch; boost_timeout_ms is the LAST-RESORT fallback, set past the slowest
    v2 apogee (~11 s, F15 half-glider) so it never pre-empts the apogee detect.

    disable_gc_flight: compact the heap at BOOSTING and hold GC OFF for the WHOLE airborne phase
    (re-enable + collect only at DONE, stationary on the ground) so no GC pause can blow a 100 Hz
    control slice (sequencer._gc_transition). False keeps GC on -- ground benches.

    apogee_arm_ms: the apogee detector (peak tracking included) is blind this long after BOOSTING
    entry -- the motor exhaust pressure wave corrupts the in-airframe baro during burn (a false peak
    could deploy GLIDING under thrust, wings still folded). Covers the longest burn (F15 3.45 s) +
    margin; no motor reaches apogee before burnout.
    ...
    stationary on the pad through SETTING (min_samples floor), freeze it at launch; the flight loop
    de-biases the wind by it. period_ms is the slow sample cadence (the drift is slow).
    """
    gnss_calib = {'name': 'gnss_calib', 'activity': 'gnss_calib', 'enabled': True,
                  'period_ms': 1000, 'min_samples': 5}

    # ---- flight: the stabilization loop, assembled from documented knob groups ----

    """
    Stabilization loop (off by default -- no actuation until enabled + tuned on the airframe).
    schedule_hz > 0 -> machine.Timer (deterministic slice, ~1 m/step at 100 Hz/100 m/s);
    schedule_hz 0 -> asyncio at period_ms. Gains/setpoint are airframe tuning; gates to GLIDING.
    """
    flight_base = {'name': 'flight', 'activity': 'flight', 'schedule_hz': 100, 'period_ms': 20,
                   'enabled': False, 'gains': {'roll': {}, 'pitch': {}, 'yaw': {}}}

    """
    bank-to-turn: GLIDING steers by banking (roll setpoint = nav_bank_gain * heading_error, capped
    ...
    using the FULL fin authority (45 deg) to crab a crosswind out -- keep it gliding, not
    rolling-and-dropping. final_approach_agl 0 -> off.

    land_bank_gain 3.0: the endgame spiral must actually REACH the 45-deg bank -- at 1.5 the
    rotating-target P-loop saturated near 25 deg and the orbit froze at ~44 m (the 25-deg turn
    radius); 45 deg gives the R_min ~20 m the <=30 m miss target needs.
    """
    final_approach = {'land_bank_gain': 3.0, 'land_bank_limit': 45, 'final_approach_agl': 8,
                      'final_cross_gain': 3.0, 'final_intercept_deg': 45}

    """
    the LOITER orbit (tuned 7/06, coludo.md "Gliding"): within loiter_capture_m of the zone centre
    the heading command becomes the circle tangent + loiter_gain deg of inward cut per metre off the
    loiter_radius_m circle -- a stable constant-radius orbit that bleeds altitude through the banked
    turn (the fly-long objective's energy management). The radius must stay ABOVE the cruise-bank
    minimum (~34 m at 30 deg) or the orbit destabilizes.
    """
    loiter = {'loiter_radius_m': 30, 'loiter_capture_m': 120, 'loiter_gain': 3.0}

    """
    the ENDGAME band: below endgame_alt_m (baro elevation) the glide steering opens the full
    land-bank authority (turn radius halves) so the last seconds SPIRAL around the zone instead of
    racetracking past it -- an in-zone/near-centre tightening that leaves the fly-long objective
    untouched up high. 0 -> off.

    ENDGAME holding pattern + airspeed-gated bank (guidance.py). endgame_pattern: 'o' a single
    circle, 'oo'/'o-o' two lobes along the long axis, or 'auto' -> the Mission picks o/oo from the
    zone aspect. NOTE 'oo' is a known-limited negative result (it overruns a narrow strip + drifts
    on wind -- see doc/sims/TMS-7-oo_landing); set 'o' for reliable in-zone landing. The gated bank
    ...
    the operator can tune them from board.config without editing source.
    """
    endgame = {'endgame_alt_m': 50, 'endgame_pattern': 'auto', 'stall_speed_1g': 9.0,
               'stall_margin': 1.2, 'endgame_max_bank': 60}
    ...
    triangle_alpha EMA-smooths the per-fix estimate; min_speed gates a noisy course at a crawl; the
    physical ENVELOPE (calm_speed .. max_speed) rejects a per-fix estimate above the ceiling as a
    GNSS jump and floors below the floor to calm. The governor reuses max_speed to reject a GNSS
    ground-speed jump (a real ground speed is within one max-wind of airspeed).
    """
    wind_estimation = {'wind': {'triangle_alpha': 0.05, 'min_speed': 3.0, 'calm_speed': 0.5,
                                'max_speed': 15.0}}

    """
    boost: BOOSTING holds the rod-vertical attitude captured at stage entry, but only once PAST THE
    ROD (airspeed > boost_engage_speed m/s) -- the 3-point rod keeps it vertical and the fins have
    no authority below that. The speed governor caps the throw the whole way up.
    """
    boost = {'boost_engage_speed': 15.0}

    """
    airspeed governor (governor.py): the estimator runs FULL RATE pre-glide and on a steep dive
    (pitch <= airspeed_dive_pitch deg -- a LEADING indicator, the dive precedes the overspeed);
    ...
    represent airspeed (vertical boost climb, steep dive) -- blending it would loosen the fin cap at
    high q. Past the gate the accel integrator flies alone (over-read = safe).
    """
    gnss_gate = {'gnss_steep_pitch': 45.0}

    """
    navigation cadence: steer()/approach() float trig recomputes at most every nav_period_ms; the
    loop reads the cached heading between (GNSS fixes ~10 Hz anyway). Omitted-but-supported knobs
    ...
    touching genuine target changes (>90 deg jumps follow at once). 0 -> off.
    """
    nav = {
    ...
    captured rod-vertical attitude (no config setpoint); GLIDING = bank-to-turn heading hold. pitch
    ...
    flight = {}
    for group in (flight_base, bank_to_turn, final_approach, loiter, endgame, reachability,
                  wind_estimation, boost, airspeed_governor, gnss_gate, nav, stage_setpoints):
        flight.update(group)  # disjoint documented groups -> one flight component (no dict-unpack: MicroPython)
    ...
    probe's sleep slice (measures wake-up lateness; the core idles between probes).
    Memory rescue (the pre-OOM safety net, measured 7/06): with GC off in flight the leak is
    garbage, an emergency gc.collect() (fins hold through the pause) reclaims the flight -- vastly
    ...
    GLIDING only (never LANDING -- the stage gate excludes the flare; no descent -> no rescue).
    `rescue: false` disables it (memory tests). oom_s + land_s ride health.csv + `inspect health`.
    """
    ...
    fallback_near_m from the pad (safety), at fallback_bearing_deg -- point that bearing at the CLEAR
    sector). Optionally AUTO-ARMS after auto_arm_dwell_s stationary with a live fix. Enable for a
