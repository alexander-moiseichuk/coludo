...
def load() -> None:
    ...
    """
    """
    List this package's OWN directory, taken from __file__ ('tasks/__init__.py' -> 'tasks'): __name__ is a
    DOTTED module name that breaks os.listdir once the package is nested, and os.path is absent on
    MicroPython (so os.path.dirname is not an option).
    """
    count = 0
