...
1. **F15, the OLD racetrack law** — steer-at-point + bank cap: 184 m leg swings across the zone,
   touchdown decided by phase luck (that sweep's median miss: 129 m).
2. **F15, loiter orbit + endgame spiral** — the tangent-capture orbit (R 30 m) holds around the
   centre, then the spiral collapses it as the altitude runs out: **18 m from the centre,
   IN-ZONE**.
3. **E16, same law** — **17 m, IN-ZONE**: the small motor lands in the zone too.
