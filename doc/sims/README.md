...
| [**TMS-7-memory_refactoring**](TMS-7-memory_refactoring/) | The **GC-off memory leak** after the Phase-3 hot-path allocation work (fixed-point PID + telemetry/`note` trims), measured from the `mem_free` slope over BOOSTING→DONE. Two rounds (normal + 50 %-weight). The directly-measured real control-path leak is **~24 KB/s → time-to-OOM ≈ 10 min** (F01 + F05), comfortably past the 180 s gate. After slimming the sim (LSM-mask bug fix + configurable `inject_hz`), the **on-device HITL leak drops 429→176 KB/s → ~185 s**, so the gate is now measurable *and green* on the board too. **Green.** | **board** (on-board HITL) |
