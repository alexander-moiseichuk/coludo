...
| [**TMS-7-memory_refactoring**](TMS-7-memory_refactoring/) | The **GC-off memory leak** after the Phase-3 hot-path allocation work (fixed-point PID + telemetry/`note` trims), measured from the `mem_free` slope over BOOSTING→DONE. Two rounds (normal + 50 %-weight). The raw HITL time-to-OOM is ~76 s, but that is ~90 % `sim_model` churn absent in real flight; the **directly-measured real control-path leak is ~24 KB/s → time-to-OOM ≈ 10 min**, comfortably past the 180 s gate. **Green** after F01 + F05; the residual airspeed-chain leak is accepted headroom. | **board** (on-board HITL) |
