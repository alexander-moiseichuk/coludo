...
_IDLE_WRITE_MS = 1000      # IDLE: buffer one summary row per second (10x decimation of the ~10 rows/s rate)
_SLICE_MS = 60000          # flush the buffer to a new file every minute of IDLE (deferred past any flight)
_LAUNCH_G = 3.0            # |a| over this (either accel) latches FLIGHT and counts as a 'busy' event (E16 ~7.5 g)
_FLIGHT_QUIET_MS = 10000   # FLIGHT ends this long after the last high-G / separation event -> flush the buffer
_BUF_MAX_ROWS = 1000       # whole-file RAM buffer cap: ~(60 s idle + 10 s flight tail) x 10 rows/s = 700, +safety
...
_PREFIX, _SUFFIX = 'burn.', '.csv'   # every new file is the next free burn.NN.csv (one per flush)

_3H = '<3h'                          # precomputed format string for 3-channel struct.unpack
_HEADER = ('t_us,phase,sep,n,dt_us,adxl_g_min,adxl_g_avg,adxl_g_max,'
...
def _row(handle, t_us, phase, sep, acc):
    """Write + flush one CSV row (IDLE / verify path)."""
    handle.write(_fmt_row(t_us, phase, sep, acc))
    try:
...
def _fmt_event(t_us, phase, adxl_g, lsm_g, lsm_dps, sep):
    """One single-sample marker line: 'start' (boot state), 'slice' (file-rotation state), or 'sep' (edge)."""
    acc = _new()
    _add(acc, adxl_g, lsm_g, lsm_dps, t_us)
    return _fmt_row(t_us, phase, sep, acc)
...
def _open(index):
    """Open burn.<index>.csv with the column header written. Returns (path, handle)."""
    out = '%s%02d%s' % (_PREFIX, index, _SUFFIX)
    handle = open(out, 'w')
...
def _flush(buf, index, t_us, adxl_g, lsm_g, lsm_dps, sep):
    ...
    handle.close()
    os.sync()
    print('wrote %s (%d records)' % (out, len(buf)))
    return [_fmt_event(t_us, 'slice', adxl_g, lsm_g, lsm_dps, sep)]


async def main():
    adxl_ok, lsm_ok = setup_sensors()
    print('logging (RAM-buffered) -- ignite within ~%ds, then pull power to end' % (_MAX_RUN_MS // 1000))
    ...
    sec_acc, win_acc = _new(), _new()      # idle 1 s summaries / flight 100 ms summaries
    n = 0
    last_idle_us = last_flush_us = t0
    phase = 'idle'
    flown = False                          # one flight capture per run; after it, just idle-log to power-off
    launch_us = sep_us = None
    deadline = 0                           # FLIGHT ends when t_us passes this; each high-G/sep shifts it +10 s
    idx = first = _first_index()           # scan once; each flush uses burn.<idx>.csv then idx += 1
    last_status_us = t0                     # 1 Hz live heartbeat to the REPL (pad debugging; no file)
    adxl_n = lsm_n = 0                      # valid (nonzero) reads per second -- a dead sensor shows 0
    # whole-file RAM buffer -- NOTHING hits flash during sampling; the file is written all at once on flush.
    buf = []
    ...
    buf.append(_fmt_event(t0, 'start', adxl_g, lsm_g, lsm_dps, prev_sep))
    while True:
        ...
        n += 1
        high_g = adxl_g > _LAUNCH_G or lsm_g > _LAUNCH_G
        sep_edge = sep != prev_sep          # latch the edge BEFORE prev_sep is updated (used again below)
        if adxl_g > 0:                      # count live reads for the heartbeat (a pulled sensor reads 0.0)
            adxl_n += 1
        if lsm_g > 0:
            lsm_n += 1

        if sep_edge:                        # record EVERY separation edge with its (uptime) timestamp
            buf.append(_fmt_event(t_us, 'sep', adxl_g, lsm_g, lsm_dps, sep))
            prev_sep = sep

        if not flown:                       # arm a single flight capture (g-spike or separation)
            if phase == 'idle' and high_g:
                phase = 'flight'
                launch_us = t_us
        ...
        flush = False
        if phase == 'flight':
            if high_g or sep_edge:          # still busy -> keep the capture window open (rolling +10 s)
                deadline = time.ticks_add(t_us, _FLIGHT_QUIET_MS * 1000)
            if n % _WINDOW == 0:
                buf.append(_fmt_row(t_us, 'flight', sep, win_acc))   # full-rate summary -> RAM
                win_acc = _new()
            if time.ticks_diff(t_us, deadline) > 0:
                phase = 'idle'              # quiet 10 s -> flight done; land it on flash NOW (a stall is safe)
                flown = True
                sec_acc = _new()
                last_idle_us = t_us
                print('FLIGHT done -- %d records buffered' % len(buf))
                flush = True
        else:
            if time.ticks_diff(t_us, last_idle_us) >= _IDLE_WRITE_MS * 1000:
                buf.append(_fmt_row(t_us, 'idle', sep, sec_acc))     # decimated 1/s summary -> RAM
                sec_acc = _new()
                last_idle_us = t_us
            if time.ticks_diff(t_us, last_flush_us) >= _SLICE_MS * 1000:
                flush = True                # periodic idle slice (only while idle -- never mid-flight)

        if len(buf) >= _BUF_MAX_ROWS:
            flush = True                    # hard RAM safety: force the whole-file write (accepts one stall)
        if flush and buf:
            buf = _flush(buf, idx, t_us, adxl_g, lsm_g, lsm_dps, sep)
            idx += 1
            last_flush_us = t_us

        if time.ticks_diff(t_us, last_status_us) >= 1000000:   # 1 Hz heartbeat: live sample counts + state
            print('status %ds: adxl=%d lsm=%d sep=%d phase=%s' % (t_us // 1000000, adxl_n, lsm_n, sep, phase))
            adxl_n = lsm_n = 0
            last_status_us = t_us

        if time.ticks_diff(t_us, t0) > _MAX_RUN_MS * 1000:
            print('timeout -- %d min cap reached' % (_MAX_RUN_MS // 60000))
            break
        try:
    ...
    if len(buf) > 1:                        # flush the tail (a raw power-pull instead loses this last buffer)
        _flush(buf, idx, t_us, adxl_g, lsm_g, lsm_dps, sep)
        idx += 1
    print('done: %d files (launch=%s sep=%s)' % (idx - first, launch_us is not None, sep_us is not None))
...
def _probe_dev(name, cs, idreg, idval, scratch):
    ...
    _wr(cs, scratch, 0x00)                        # restore the register's default
    responds = (idval in ids) or (echo == 0x7F)
    verdict = 'RESPONDS' if responds else ('SILENT (constant 0x%02X)' % ids[0] if len(set(ids)) == 1 else 'FLAKY')
    print('%-9s id=[%s] want %02X | scratch 0x%02X echo=%02X(%s) -> %s' % (
        name, ' '.join('%02X' % v for v in ids), idval, scratch, echo,
        'ok' if echo == 0x7F else 'no', verdict))
    return responds
...
def probe():
    ...
    if adxl_ok and lsm_ok:
        print('verdict: both OK -- bus + parts fine; any earlier 0 g was transient, re-run the logger')
    elif lsm_ok:
        print('verdict: bus OK (LSM responds), ADXL silent -> ADXL branch open: its CS(GPIO%d)/3V3/GND or a '
              'pulled lead (e.g. at "separation"). Reseat the ADXL wiring.' % _ADXL_CS)
    elif adxl_ok:
        print('verdict: bus OK (ADXL responds), LSM silent -> LSM branch open: its CS(GPIO%d)/3V3/GND or a '
              'pulled lead. Reseat the LSM wiring.' % _LSM_CS)
    else:
        print('verdict: NEITHER responds -> shared-bus fault (SCK/MOSI/MISO/3V3/GND) or both unpowered')
    return adxl_ok, lsm_ok
